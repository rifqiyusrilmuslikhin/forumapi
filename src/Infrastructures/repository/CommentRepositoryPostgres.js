...
class CommentRepositoryPostgres extends CommentRepository {
  ...
  async addComment(newComment) {
    ...
    const query = {
      text: 'INSERT INTO comments VALUES($1, $2, $3, $4, FALSE ,$5, $6) RETURNING id, content, owner',
  ...
  async getCommentsThread(thread) {
    const query = {
      text: 'SELECT comments.id, users.username, comments.created_at as date, comments.content, comments.is_deleted FROM comments JOIN users ON users.id = comments.owner WHERE thread = $1 ORDER BY comments.created_at ASC',
      values: [thread],
    };
  ...
  //   const query = {
  //     text: 'SELECT id FROM comments WHERE is_delete = false AND id = $1',
  //     values: [comment],
  //   };
  //   const result = await this._pool.query(query);
